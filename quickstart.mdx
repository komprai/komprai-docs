---
title: "Quickstart"
description: "Autentique e faça sua primeira chamada à API Komprai"
---

## Pré-requisitos

- Conta criada no [console.komprai.dev](https://console.komprai.dev)
- Acesso a um terminal com `curl` ou cliente HTTP

## Passo 1 — Autenticação

Faça login com email e senha para obter a sessão:

```bash
curl -c cookies.txt -X POST https://api.main.komprai.dev/auth/sign-in/email \
  -H "Content-Type: application/json" \
  -d '{"email": "seu@email.com", "password": "sua-senha"}'
```

O cookie `better-auth.session_token` será salvo em `cookies.txt` e usado automaticamente nos próximos requests.

## Passo 2 — Busque seu usuário

```bash
curl -b cookies.txt https://api.main.komprai.dev/api/users/me
```

Resposta esperada:

```json
{
  "user": {
    "id": "...",
    "email": "seu@email.com",
    "name": "Seu Nome",
    "role": "user"
  },
  "session": { ... }
}
```

## Passo 3 — Crie uma store

```bash
curl -b cookies.txt -X POST https://api.main.komprai.dev/api/stores \
  -H "Content-Type: application/json" \
  -d '{"name": "Minha Loja", "active": true}'
```

## Passo 4 — Consulte ofertas de uma região

```bash
curl -b cookies.txt https://api.sa-east-1.komprai.dev/api/catalog/offers
```

<Tip>
  Cada região tem sua própria instância do Catalog API. Use `api.sa-east-1.komprai.dev` para São Paulo ou `api.us-east-1.komprai.dev` para North Virginia.
</Tip>

## Próximos passos

<CardGroup cols={2}>
  <Card title="Core API" icon="server" href="/core-api">
    Explore stores, providers, billing e SSO.
  </Card>
  <Card title="Catalog API" icon="tag" href="/catalog-api">
    Publique e consulte ofertas por região.
  </Card>
</CardGroup>
